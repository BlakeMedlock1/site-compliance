...
export const BuildingServices = ({ navigation }: any) => {
  const { user } = useAuth();
  const { services, loading, handleDuplicate } = useBuildingServices(navigation, user);
        ...
        {isManager && (
          <View style={styles.actionRow}>
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate("ContractorAssignment", {
                incidentId: item.id,
                isAsset: true,
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: COLORS.white, padding: SPACING.m, borderRadius: 12, marginBottom: SPACING.m, borderLeftWidth: 8, ...SHADOWS.light, minHeight: 120 },
  ...
  dateGrid: { flexDirection: "row", justifyContent: "space-between", marginTop: SPACING.m, paddingTop: SPACING.s, borderTopWidth: 1.5, borderTopColor: COLORS.lightGray },
  ...
  assignBtn: { backgroundColor: COLORS.primary, flex: 0.75, minHeight: TOUCH_TARGETS.min, borderRadius: 10, alignItems: "center", justifyContent: "center" },
  copyBtn: { backgroundColor: COLORS.lightGray, flex: 0.25, minHeight: TOUCH_TARGETS.min, borderRadius: 10, alignItems: "center", justifyContent: "center" },
  ...
  navAddBtn: { backgroundColor: COLORS.success, paddingHorizontal: 12, minHeight: 36, borderRadius: 8, justifyContent: "center", marginRight: 10 },
